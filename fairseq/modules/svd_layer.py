...
from fairseq.modules import LayerNorm, SVDMultiheadAttention #MultiheadAttention
...
from fairseq.modules import multihead_svd # The actual synth code. TODO: merge inside synthesized_multihead_attention

class SVDDecoderLayerBase(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.multihead_svd_attn = multihead_svd.SVDManualMH(self.embed_dim, SENTENCE_LENGTH_HARDCODE, self.nh) # Goal: create [64, 512, 512, 8]
    ...
    def build_encoder_attention(self, embed_dim, cfg):
        return SVDMultiheadAttention( # TODO check, should this also be Synth?
            embed_dim,
    ...
    def forward(
        ...
        # decoder, transformer, transformer layer
        # Replace attn in transformerDecoderLayerBase, the rest can be the same
        # Build new class synth, inherits from transformer decoder, change where calls 
...
# backward compatible with the legacy argparse format
class SVDDecoderLayer(SVDDecoderLayerBase):
    def __init__(
